...
async def auth(client, message: Message, _):
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text("**» اكتب ما تريد تشغيله/ᴜsᴇʀ_ɪᴅ.**")
        ...
        count = len(_check)
        if int(count) == 20:
            return await message.reply_text("**» يمكن ان يكون لديك 20 مستخدم في هذه القائمة (ᴀᴜʟ).**")
        if token not in _check:
            ...
            return await message.reply_text("**» اضف الى قائمة المستخدمين من تريده من مجموعتك.**")
        else:
            await message.reply_text("**» موجود في قائمة المستخدمين.**")
        return
    from_user_id = message.from_user.id
...
async def unauthusers(client, message: Message, _):
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text("**» اكتب ما تريد تشغليه/ᴜsᴇʀ_ɪᴅ.**")
        user = message.text.split(None, 1)[1]
        if "@" in user:
            user = user.replace("@", "")
        user = await app.get_users(user)
        ...
        if deleted:
            ...
            return await message.reply_text("**» موجود في قائمة المستخدمين.**")
    user_id = message.reply_to_message.from_user.id
    token = await int_to_alpha(user_id)
    deleted = await delete_authuser(message.chat.id, token)
    get = adminlist.get(message.chat.id)
